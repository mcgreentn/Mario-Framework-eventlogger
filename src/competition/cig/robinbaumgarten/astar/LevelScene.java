...
public class LevelScene implements SpriteContext, Cloneable {
	...
	for (int y = MarioYInMap - HalfObsHeight, obsX = 0; y < MarioYInMap + HalfObsHeight; y++, obsX++) {
		...
		if (x >= 0 && x <= level.xExit && y >= 0 && y < level.height) {
		    if(GlobalOptions.SceneGeneration && x >= 20) {
			continue;
		    }
		    byte datum = data[obsX][obsY];
		    /*
		     * byte value = 0; // inverse of LevelScene.ZLevelMapElementGeneralization
		     * switch (datum) { case 0: value = -106; break; case -10: value = 4;break; case
		     * -12: value = 9;break; case -11: value = -123;break;//-76;break; case 20:
		     * value = 10;break; case 95: value = -106;break;
		     * 
		     * default: value = datum; }
		     */
	...
	if (gapAtLast && !gapAtSecondLast) {
		...
		int tempIndex = MarioXInMap + HalfObsWidth + i;
		if (tempIndex >= 0 && tempIndex < level.isGap.length) {
		    level.isGap[tempIndex] = true;
		    level.gapHeight[tempIndex] = gapBorderMinusOneHeight;
		}
	    }
	...
	if (verbose > 1)
	    System.out.println("Enemies: " + enemies.length);
	for (int i = 0; i < enemies.length; i += 3) {
		...
		winged = true;
		break;
	    case (Sprite.KIND_FLOWER_ENEMY):
		type = Enemy.ENEMY_FLOWER;
		break;
	    }
	    if (!(type == -1 && kind == 0)) {
		if (verbose > 6)
		...
		if (sprite.kind == kind)
		    if (verbose > 6)
			System.out.println(
				"Same kind Sprite found! pos: " + sprite.x + " " + sprite.y + " xa: " + sprite.xa);
		...
		if (sprite.kind == kind && (sprite.x - x) == 0 && (sprite.y - y) != 0 && Math.abs(sprite.y - y) < 8
			&& sprite.kind != Sprite.KIND_SHELL && sprite.kind != Sprite.KIND_BULLET_BILL
			&& ((Enemy) sprite).winged) {
		...
		if (sprite.kind == kind && (sprite.x - x) == 0 && (sprite.y - y) != 0 && Math.abs(sprite.y - y) <= 2
			&& sprite.unknownYA && sprite.lastAccurateY != 0) {
