...
//    public int targetY = 100;
	...
	public float estimateRemainingTimeChild(boolean[] action, int repetitions) {
	    float[] childbehaviorDistanceAndSpeed = estimateMaximumForwardMovement(levelScene.mario.xa, action,
		    repetitions);
...
//	    System.out.println("Before " + printAction(action) + " " + parentPos.sceneSnapshot.mario.xa + " " + levelScene.mario.xa);
//	    System.out.println("Before " + printAction(action) + " " + parentPos.sceneSnapshot.mario.damage + " " + levelScene.mario.damage);
	    int initialDamage = getMarioDamage();
	    for (int i = 0; i < repetitions; i++) {
		/*
		 * if (debugPos < 1000) { GlobalOptions.Pos[debugPos][0] = (int)
		 * levelScene.mario.x; GlobalOptions.Pos[debugPos][1] = (int)
		 * levelScene.mario.y; debugPos++; }
		 */
		advanceStep(action);
		/*
		 * if (debugPos < 1000) { GlobalOptions.Pos[debugPos][0] = (int)
...
//	 possibleActions.add(createAction(false, false, false, false, false));
	...
	for (int i = 0; i < ticks; i++) {
	    ...
	    dist += currentAccel;
	    // System.out.println("Estimator of Fastforward Speed, Tick "+i+" speed:
	    // "+currentAccel);
	    currentAccel *= 0.89f;
	}
    ...
    // distance covered at maximum acceleration with initialSpeed for ticks
    // timesteps
...
//	    ticks++;
	    ...
	    // System.out.println("Simulated mariopos: "+ current.sceneSnapshot.mario.x + "
	    // " + current.sceneSnapshot.mario.y);
	    if (realRemainingTime < 0) {
		System.out.print("-");
		continue;
	    } else if (!current.isInVisitedList && isInVisited((int) current.sceneSnapshot.mario.x,
		    (int) current.sceneSnapshot.mario.y, current.timeElapsed)) {
		if (levelScene.verbose > 12)
		    System.out.print("V");
...
//		}
		...
		/*
		 * current.pathQuality -= 100; realRemainingDist -= 100;
		...
		if (levelScene.verbose > 12)
		    System.out.print("b");
		// current item is not as good as anticipated. put it back in pool and look for
		// best again
		current.remainingTimeEstimated = realRemainingTime;
		posPool.add(current);
	    } else {
	...
	// bestPosition = current;
	if (levelScene.verbose > 1)
	    System.out.println("Search stopped. Remaining pool size: " + posPool.size() + " Current remaining time: "
	...
	/*
	 * if (bestPosition != null) { LevelScene internal = backupState(); // insert
	 * old plan SearchNode pos = bestPosition; // rewind int steps = 0; while
	 * (pos.parentPos != null) { steps++; pos = pos.parentPos; } if (steps > 3) {
	 * pos = pos.chosenChild.chosenChild.chosenChild; // go forward if (pos != null
	 * && pos.parentPos != null) { pos.parentPos.sceneSnapshot = backupState(); //
	 * overwrite previous scenesnapshot //float previousRemainingTime =
	 * pos.remainingTime; while(pos != null && pos.remainingTime ==
	 * pos.simulatePos()) { // same conditions SearchNode newP = new
	 * SearchNode(pos.action, pos.repetitions, pos.parentPos); newP.sceneSnapshot =
	 * pos.sceneSnapshot; newP.remainingTime = pos.remainingTime;
	 * newP.remainingTimeEstimated = pos.remainingTimeEstimated; posPool.add(newP);
	 * //posPool.addAll(newP.generateChildren()); pos = pos.chosenChild;
	 * 
	 * } } restoreState(internal); } }
	 * 
	 * 
	 * for(int i = 0; i < 1000; i++) { GlobalOptions.Pos[i][0] = 0;
	...
	while (current.parentPos != null) {
	    ...
	    if (levelScene.verbose > 2)
		System.out.print("[" + (current.action[Mario.KEY_DOWN] ? "d" : "")
			+ (current.action[Mario.KEY_RIGHT] ? "r" : "") + (current.action[Mario.KEY_LEFT] ? "l" : "")
			+ (current.action[Mario.KEY_JUMP] ? "j" : "") + (current.action[Mario.KEY_SPEED] ? "s" : "")
			+ (current.hasBeenHurt ? "-" : "") + "]");
...
//		}
	    ...
	    // jumpModifier += 0.001f * (300-current.sceneSnapshot.mario.y);
	    float currentCost = current.getRemainingTime() + current.timeElapsed * 0.90f + jumpModifier; // slightly
													 // bias towards
													 // furthest
													 // positions
...
//	}
	...
	if (levelScene.verbose > 8)
	    System.out.print("[" + (action[Mario.KEY_DOWN] ? "d" : "") + (action[Mario.KEY_RIGHT] ? "r" : "")
		    + (action[Mario.KEY_LEFT] ? "l" : "") + (action[Mario.KEY_JUMP] ? "j" : "")
	...
	if (ticksBeforeReplanning <= 0 || currentActionPlan.size() == 0 || requireReplanning) {
		...
		if (levelScene.verbose > 2)
		    System.out.println("Warning!! currentActionPlan smaller than planAhead! plansize: "
			    + currentActionPlan.size());
	...
	if (levelScene.verbose > 0)
	    System.out.println("Simulation took " + (e - startTime) + "ms.");
