...
import ch.idsia.mario.engine.EventLogger;
	...
	public void move() {
		...
		if (keys[KEY_JUMP] || (jumpTime < 0 && !onGround && !sliding)) {
			if (jumpTime < 0) {
				...
				facing = -facing;
				//TODO
				System.out.println("sliding_jump");
			} else if (jumpTime > 0) {
				xa += xJumpSpeed;
				ya = jumpTime * yJumpSpeed;
				jumpTime--;
		...
		if (!onGround) {
			...
			if (startX > 0 && Math.floor(Math.abs(x - startX)) > 0) {
				...
				if ((float) Math.floor(Math.abs(minY - startY)) >= 80.0) {
					EventLogger.addToLogger("Long Jump", this.getMode(), this.x, this.y, this.world.getStartTime());
				} else {
					EventLogger.addToLogger("Short Jump", this.getMode(), this.x, this.y, this.world.getStartTime());
				}
			}
			if(startY > 0 && Math.floor(Math.abs(minY - startY)) > 0) {
				LevelScene.highJump.add((float) Math.floor(Math.abs(minY - startY)));
				if ((float) Math.floor(Math.abs(minY - startY)) >= 66.0) {
					EventLogger.addToLogger("High Jump", this.getMode(), this.x, this.y, this.world.getStartTime());
				} else {
					EventLogger.addToLogger("Low Jump", this.getMode(), this.x, this.y, this.world.getStartTime());
				}
			}
			startX = -1;
			startY = -1;
		...
		if (keys[KEY_RIGHT] && !ducking) {
			if (facing == -1)
				sliding = false;
			xa += sideWaysSpeed;
//			EventLogger.addToLogger("move_right", world.getStartTime());
			if (jumpTime >= 0)
				facing = 1;
		}
	...
	private boolean isBlocking(float _x, float _y, float xa, float ya) {
		...
		if (((Level.TILE_BEHAVIORS[block & 0xff]) & Level.BIT_PICKUPABLE) > 0) {
			Mario.getCoin(this);
			EventLogger.addToLogger("Coin Collected From Floor", this.getMode(), this.x, this.y, this.world.getStartTime());
	...
	public void stomp(Enemy enemy) {
		...
		EventLogger.addToLogger("Enemy Stomp", this.getMode(), this.x, this.y, this.world.getStartTime());
	}
	...
	public void getHurt() {
		if (deathTime > 0 || world.paused || isMarioInvulnerable)
			return;
	...
	public void getFlower() {
		...
		EventLogger.addToLogger("Flower Powerup Collected From Block", this.getMode(), this.x, this.y, this.world.getStartTime());
		++gainedFlowers;
	}
	...
	public void getMushroom() {
		...
		EventLogger.addToLogger("Mushroom Powerup Collected From Block", this.getMode(), this.x, this.y, this.world.getStartTime());
		++gainedMushrooms;
	}
